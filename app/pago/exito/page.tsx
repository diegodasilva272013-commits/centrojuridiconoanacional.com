  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-5 py-16 bg-[#f0fdf4]">
      <div className="bg-white rounded-2xl border border-[#d1fae5] shadow-card px-8 py-10 max-w-md w-full">
        ...
        {/* ── Bloque MercadoPago: compra aprobada ── */}
        <div className="flex flex-col items-center text-center mb-8 pb-8 border-b border-[#d1fae5]">
          ...
          <p className="text-sm text-[#16a34a] font-semibold mb-2">
          ...
          {(paymentId || status) && (
            <div className="inline-flex flex-col gap-1 bg-[#f0fdf4] rounded-xl px-4 py-3 text-xs text-gray-500 mt-2">
              {status && (
                <span>
                  Estado:{" "}
                  <span className="font-semibold text-[#16a34a] capitalize">{status === "approved" ? "Aprobado ✓" : status}</span>
                </span>
              )}
              {paymentId && (
                <span>
                  N° de pago: <span className="font-semibold text-gray-700">{paymentId}</span>
                </span>
              )}
            </div>
          )}
        ...
        {/* ── Formulario de entrega o confirmación final ── */}
        {done ? (
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <p className="text-sm text-metal-mid">
              Recibimos tus datos. En breve te enviamos la guía por WhatsApp al número que indicaste.
            </p>
            ...
            <Link
              href="/"
              className="mt-2 inline-block bg-carbon text-white rounded-xl px-6 py-3 text-sm font-semibold hover:opacity-80 transition-opacity"
            >
              Volver al inicio
            </Link>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-metal-mid mb-6">
              Ingresá tus datos para enviarte la guía por WhatsApp.
            </p>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-2 w-full inline-flex items-center justify-center gap-2 bg-[#16a34a] hover:bg-[#15803d] text-white font-semibold rounded-xl min-h-[52px] px-6 py-3 text-sm sm:text-base transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
